...
| `LANGFLOW_ASSISTANT_DEFAULT_MODEL` | String | Not set | The model [Langflow Assistant](/langflow-assistant) selects by default, as `Provider:model`, for example `OpenAI:gpt-5.4`. When not set, the Assistant panel selects the first capable model in catalog order, which is the provider's most recent model and usually its most expensive one. Set this for deployments whose users don't pick a model themselves. It applies only while that model is enabled for the user, and a model that the user selects in the panel always takes precedence. |
| `LANGFLOW_ASSISTANT_ASK_MODEL` | String | Not set | The model [Langflow Assistant](/langflow-assistant) uses for **Ask** mode, as `Provider:model`. **Ask** mode answers questions from the documentation and never changes the canvas, so it can run on a much cheaper model than **Build** mode, which drives a long agent loop. When not set, or when that provider isn't configured for the user, **Ask** mode uses the model selected in the panel. |
| `LANGFLOW_ASSISTANT_AUTO_APPLY_DEFAULT` | Boolean | `False` | Whether [Langflow Assistant](/langflow-assistant) applies plans and flow proposals without asking, for users who haven't chosen either way. An applied flow keeps a **Revert** action in the chat. Generated components still require approval. Users can change this with **Apply changes automatically** in the Assistant panel menu, and their choice takes precedence. |
| `LANGFLOW_ASSISTANT_DOCK_DEFAULT` | Boolean | `False` | Whether the [Langflow Assistant](/langflow-assistant) panel opens docked beside the canvas instead of floating over it, for users who haven't chosen either way. A docked panel stays open while you work on the canvas. The user's own choice takes precedence. |
| `LANGFLOW_ASSISTANT_DOCS_BASE_URL` | String | `https://docs.langflow.org` | The base URL of the documentation links that [Langflow Assistant](/langflow-assistant) cites in **Ask** mode. **Ask** mode searches a documentation index that ships with Langflow and needs no internet access; only the cited links point at this URL. Set it to an internal mirror for deployments without internet access. |
