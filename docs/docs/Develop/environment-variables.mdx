...
| `LANGFLOW_ASSISTANT_ASK_MODEL` | String | Not set | The model [Langflow Assistant](/langflow-assistant) uses in the **Ask** tab, as `Provider:model`. **Ask** answers questions from the documentation and never changes the canvas, so it can run on a much cheaper model than the **Component** and **Prompt** tabs, which write code and instructions. When not set, or when that provider isn't configured for the user, **Ask** uses the model selected in the panel. |
