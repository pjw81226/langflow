...
describe("useAssistantChat", () => {
  ...
  describe("handleSend", () => {
    ...
    it("should abort a stream that is still open when a new send starts", async () => {
      // A malformed event ends the turn in the UI through onError, but the
      // stream keeps reading; a leaked reader would write into the next turn.
      mockPostAssistStream.mockImplementationOnce(
        (_request: unknown, callbacks: Record<string, Function>) => {
          callbacks.onError({ event: "error", message: "malformed" });
          return new Promise(() => {});
        },
      );
      ...
      await act(async () => {
        void result.current.handleSend("first message", TEST_MODEL);
      });
    ...
    it("should_treat_a_missing_validated_flag_as_not_validated", async () => {
      mockPostAssistStream.mockImplementation(
        async (_request: unknown, callbacks: Record<string, Function>) => {
          callbacks.onComplete({
            event: "complete",
            data: { result: "An answer.", notices: [] },
          });
        },
      );
      ...
      expect(result.current.messages[1].result?.validated).toBe(false);
