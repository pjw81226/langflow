...
Usage (from anywhere, no virtualenv required):
    python3 scripts/i18n/check_ko.py
    python3 scripts/i18n/check_ko.py --only frontend
    python3 scripts/i18n/check_ko.py --file part.json --against frontend
"""
...
BANNED_CHARS = {"—": "em dash", "–": "en dash", "·": "middle dot", "§": "section sign"}
...
TRANSLATIONESE = [
    ...
    (r"(?:파일|항목|설정|데이터|결과|옵션|메시지|값|키|노드|문서|기능|요청|오류|세션|버전|폴더|변수|필드|링크)들", "no plural 들 on things"),
    (r"다양한|혁신적|강력한|원활한|손쉽게|효율적으로", "empty modifier"),
    (r"을\(를\)|를\(을\)|이\(가\)|가\(이\)|은\(는\)|는\(은\)|와\(과\)|과\(와\)|\(으\)로", "no paired particles, reorder the sentence"),
    (r"\}\}[\"'”’)\]]?(?:을|를|이|가|은|는|와|과|로|으로)(?=[\s.,?!]|$)", "no batchim-dependent particle right after a variable"),
...
def check_value(report: Report, key: str, en_value: str, ko_value: str, *, markdown: bool) -> None:
    ...
    if not HANGUL.search(ko_value) and len(en_value.split()) >= 4 and ko_value == en_value:
        report.warn(key, f"looks untranslated | {ko_value[:60]}")
    if not markdown and len(en_value) <= 24 and width(ko_value) > 2 * len(en_value) + 6:
